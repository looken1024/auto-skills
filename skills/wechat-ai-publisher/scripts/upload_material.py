...
功能：
- 上传图片到微信服务器
- 返回 thumb_media_id 用于创建文章封面

授权方式: 使用 app_id 和 app_secret 获取 access_token
...
def upload_material(app_id=None, app_secret=None, image_path=None):
    ...
    try:
        with open(image_path, 'rb') as f:
            ...
            # 5. 发起上传请求（带网络重试）
            response = request_with_retry('POST', url, files=files, timeout=60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="上传图片素材到微信公众号（带网络重试）",
