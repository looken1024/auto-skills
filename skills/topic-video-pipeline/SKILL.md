...
- 顶部红色大标题（白描边）：fontsize 56（2026-08-29 用户确认缩小，宽度约 68-73% 屏宽；可 TITLE_FS 覆盖），y=h*0.106（约占屏高 4.3%）
- 副标题黄色底框黑字：fontsize 52、line_spacing=18（两行默认），**y=h*0.165（与主标题间隔约 41px，2026-08-29 用户确认"紧贴但有间隔"，勿改回 0.215/0.156）**，boxborderw=16
...
**标题参数（2026-08-29 最新）**：`make_vertical.sh` 支持环境变量 `TITLE_FS`（主标题字号，默认56）、`SUB_FONTSIZE`（默认52）、`SUB_LINE_SPACING`（默认18，两行）、`SUB_Y`（副标题顶部 y 比例，默认0.165 → 主副标题间距约41px）。两行副标题用：`bash make_vertical.sh <素材> <标题> "第一行\n第二行" <输出> [时长] [bgm]`（默认即两行参数）；单行用 `SUB_FONTSIZE=75 SUB_LINE_SPACING=0 SUB_Y=0.18`。
